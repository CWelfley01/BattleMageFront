...
export default class Hud extends Component {
  ...
  combineElements = () => {
    ...
    axios.get(`https://battlemage.herokuapp.com/Element`).then((response) => {
      this.setState({
  ...
  createBSpell = () => {
    ...
    axios.get(`https://battlemage.herokuapp.com/Form`).then((response) => {
      this.setState({
        spell: response.data
          .filter((item) =>
  ...
  createASpell = () => {
    ...
    axios.get(`https://battlemage.herokuapp.com/`).then((response) => {
      this.setState({
        spell: response.data
          .filter((item) =>
    ...
    if (this.state.element1 !== "blank") {
      if (prevState.element2 !== this.state.element2) {
        if (this.state.element1 === this.state.element2) {
          this.setState({spell: "BANG!!! Your spell backfired and killed you!!"});
        } else {
